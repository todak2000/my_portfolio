  ...
  const handleVisitorData = async () => {
    try {
      if (uniqueUser === true) {
        ...
        await setDoc(visitorRef, { count: increment(1) }, { merge: true }).then(
          (res) => console.log("unique visitor data added to store")
        );
      } else if (uniqueUser === false) {
        const visitorRef = doc(db, "app_visitor_data", "Regular");
        await setDoc(visitorRef, { count: increment(1) }, { merge: true }).then(
          (res) => console.log("regular visitor data added to store")
        );
      }
    } catch (error) {
